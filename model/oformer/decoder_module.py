...
class IrregSTDecoder3D(nn.Module):
    def __init__(self,
                 max_node_type,
                 latent_channels,  # 256??
                 out_channels,  # 1 or 2?
        ...
        self.propagator = nn.ModuleList([
            nn.ModuleList([nn.LayerNorm(self.latent_channels*2),
                           nn.Sequential(
                               nn.Linear(self.latent_channels*3 + 3, self.latent_channels*2, bias=False),
                               nn.GELU(),
                               nn.Linear(self.latent_channels*2, self.latent_channels*2, bias=False),
                               nn.GELU(),
                               nn.Linear(self.latent_channels*2, self.latent_channels*2, bias=False),
...
class MarkovianIrregSTDecoder2D(nn.Module):
    def __init__(self,
                 max_node_type,
                 latent_channels,  # 256??
                 out_channels,  # 1 or 2?
        ...
        self.out_norm = nn.LayerNorm(self.latent_channels)
        self.to_out = nn.Sequential(
            nn.Linear(self.latent_channels*2, self.latent_channels*2, bias=False),
            nn.ReLU(),
            nn.Linear(self.latent_channels*2, self.latent_channels, bias=False),
