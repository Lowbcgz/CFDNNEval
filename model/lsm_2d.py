...
class LSM_2d(nn.Module):
    def __init__(self, inputs_channel, outputs_channel=None, d_model=32, num_token=4, num_basis=12, initial_step=1, n_case_params = 5, patch_size="",padding="",bilinear=True):
        ...
        in_channels = inputs_channel*initial_step+1+2+n_case_params  # +1 for the mask, +2 for the grids
    ...
    def forward(self, x, case_params, mask, grid):
        _, H, W, _ = x.shape
        assert (H+self.padding[1]) % 16 == 0 and (W+self.padding[0]) % 16 == 0, f"padding ({self.padding[0]},{self.padding[1]}) is not suitable for (H,W)=({H},{W}), the shape after padding should be divisible by 16."
        assert (H+self.padding[1]) % self.patch_size[0] == 0 and (W+self.padding[0]) % self.patch_size[1] == 0
