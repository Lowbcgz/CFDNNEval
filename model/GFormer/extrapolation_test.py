from utils import get_min_max, setup_seed, process_data
from NS_train import get_model, get_dataset, get_dataloader
import metrics
...
def main(args):
    ...
    checkpoint = None
    saved_dir = os.path.join(args["saved_dir"], os.path.join(args["model_name"], args['dataset']["flow_name"] + '_' + args['dataset']['case_name']))
    
    fig_dir = os.path.join(args['output_dir'], 'extp_fig')
    fig_dir = os.path.join(fig_dir, os.path.join(args["model_name"],args['dataset']["flow_name"] + '_' + args['dataset']['case_name']))

    error_dir = os.path.join(args['output_dir'], 'extp_error')
    error_dir = os.path.join(error_dir, os.path.join(args["model_name"],args['dataset']["flow_name"] + '_' + args['dataset']['case_name']))
    ...
    if args['dataset']["flow_name"] == "cylinder":
    ...
    elif args['dataset']["flow_name"] == "ircylinder":
...
if __name__ == "__main__":
    ...
    dataset_args= args["dataset"]
    assert dataset_args["flow_name"] in ["cylinder", "ircylinder"]
