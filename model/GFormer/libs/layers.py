...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, n_1, n_2, in_dim)
        # 1：线性变换
        res = self.linear(x)
        # 2：傅里叶变换与逆变换
        # 将输入从空间域转换到频域，进行卷积操作，然后再将结果转换回空间域
        x = self.dropout(x)

        x = x.permute(0, 3, 1, 2)
        x_ft = fft.rfft2(x, s=(n_1, n_2), norm=self.norm)  # 二维实部快速傅里叶变换（RFFT）。将输入数据从空间域转换到频域，并返回频域中的实部和虚部
        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=-1)  # 将实部和虚部堆叠起来，形成一个新的张量。

        out_ft = torch.zeros(batch_size, out_dim, n_1, n_2 //
                             2+1, 2, device=x.device)
        out_ft[:, :, :modes, :modes] = self.complex_matmul_2d(
            x_ft[:, :, :modes, :modes], self.fourier_weight[0])  # 频域表示的前 modes 个频率分量，与第一个谱权重 self.fourier_weight[0] 进行复数矩阵相乘
        out_ft[:, :, -modes:, :modes] = self.complex_matmul_2d(
            x_ft[:, :, -modes:, :modes], self.fourier_weight[1])  # 频域表示的后 modes 个频率分量，与第一个谱权重 self.fourier_weight[1] 进行复数矩阵相乘
        out_ft = torch.complex(out_ft[..., 0], out_ft[..., 1])

        x = fft.irfft2(out_ft, s=(n_1, n_2), norm=self.norm)  # 对复数张量进行二维逆快速傅里叶变换（IRFFT），将频域表示转换回空间域
        x = x.permute(0, 2, 3, 1)
        x = self.activation(x + res)
...
class SpectralConv3d(nn.Module):
    def __init__(self, in_dim, out_dim, 
        ...
        self.n_grid = n_grid  # just for debugging
        self.fourier_weight = nn.ParameterList([Parameter(
            torch.FloatTensor(in_dim, out_dim,modes,
                                                modes, modes, 2)) for _ in range(2)])
        for param in self.fourier_weight:
            xavier_normal_(param, gain=1/(in_dim*out_dim)
                           * np.sqrt(in_dim+out_dim))
    ...
    def complex_matmul_3d(self, a, b):
    ...
    def forward(self, x):
        '''
        Input: (-1, n_grid**3, in_features) or (-1, n_grid, n_grid, n_grid, in_features)
        Output: (-1, n_grid**3, out_features) or (-1, n_grid, n_grid, n_grid, out_features)
        '''
        # print('in SpectralConv3d-----------------------------------------------------------------------')
        batch_size = x.size(0)
        n_dim = x.ndim
        ...
        x = x.permute(0, 4, 1, 2, 3)  # (bs, out_dim, n, n, n)
        x_ft = fft.rfftn(x, s=(n, n, n), norm=self.norm)  # 三维实部快速傅里叶变换（RFFT）。将输入数据从空间域转换到频域，并返回频域中的实部和虚部
        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=-1)  # 将实部和虚部堆叠起来，形成一个新的张量。
        # print('after fft:', x_ft.shape)

        out_ft = torch.zeros(batch_size, out_dim, n, n, n //  # 傅里叶变换仅最后一个维度变了？
                             2+1, 2, device=x.device)
        out_ft[:, :, :modes, :modes, :modes] = self.complex_matmul_3d(
            x_ft[:, :, :modes, :modes, :modes], self.fourier_weight[0])  # 频域表示的前 modes 个频率分量，与第一个谱权重 self.fourier_weight[0] 进行复数矩阵相乘
        out_ft[:, :, -modes:, :modes, :modes] = self.complex_matmul_3d(
            x_ft[:, :, -modes:, :modes, :modes], self.fourier_weight[1])  # 频域表示的后 modes 个频率分量，与第一个谱权重 self.fourier_weight[1] 进行复数矩阵相乘
        out_ft = torch.complex(out_ft[..., 0], out_ft[..., 1])
        # print('after mode:', out_ft.shape)

        x = fft.irfftn(out_ft, s=(n, n, n), norm=self.norm)  # 对复数张量进行二维逆快速傅里叶变换（IRFFT），将频域表示转换回空间域
        # print('after fft2:', x.shape)
        x = x.permute(0, 2, 3, 4, 1)
        x = self.activation(x + res)

        if n_dim == 4:
            x = x.view(batch_size, n**3, out_dim)
