...
class LSM_2d_ir(nn.Module):
    def __init__(self, inputs_channel, outputs_channel=None, d_model=32, num_token=4, num_basis=12, patch_size="",
                 initial_step=1, n_case_params = 5, use_iphi= True, 
                 bilinear=True, modes1=12, modes2=12, s1=96, s2=96):
        super(LSM_2d_ir, self).__init__()
        in_channels = inputs_channel*initial_step+1+2+n_case_params  # +1 for the mask, +2 for the coordinates
    ...
    def forward(self, x, case_params, mask, cords):
        # cords : [bs, Nx, 2] , where 2D cordinates should be in [0, 1]^2
        x = torch.cat((x, mask, cords,  case_params), dim=-1)
        cords_in = cords
        cords_out = cords
        grid = self.get_grid([x.shape[0], self.s1, self.s2], x.device).permute(0, 3, 1, 2)  # regular grid in [0,1]^2

        u = self.fc0(x)
        u = u.permute(0, 2, 1)

        uc1 = self.fftproject_in(u, x_in=cords_in, iphi=self.iphi, code=None)
        ...
        u = self.fftproject_out(uc, x_out=cords_out, iphi=self.iphi, code=None)
        u1 = self.convproject_out(cords_out.permute(0, 2, 1))
        u = u + u1
