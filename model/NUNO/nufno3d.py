...
class NUNO_Data_utils:
    ...
    def _data_preprocessing(self, x,  grid, tree, n_subdomains=8):
        ...
        with torch.no_grad():
            ...
            input_point_cloud = x    # shape: (1000, 19517, 5)
        ...
        xyz_sd = xyz_sd\
                .transpose((0, 2, 1, 3))\
                .reshape(n_subdomains, -1, 1, 1, 3)
                # shape: (n_subdomains, n_points_sd_padded, 1, 1, 3)
        input_point_cloud_sd = input_point_cloud_sd.transpose((0, 1, 3, 2))
                # shape: (ntotal, n_points_sd_padded, n_subdomains, output_dim)
        input_u_sd_mask = torch.from_numpy(input_u_sd_mask).permute(0, 1, 3, 2).repeat([batch_size, 1, 1, 1]).float()
                # shape: (1, n_points_sd_padded, n_subdomains, 1)
        input_sd_grid = input_sd_grid.transpose((0, 1, 2, 3, 5, 4))
        # print(input_u_sd_mask.shape, input_point_cloud_sd.shape)
                # shape: (n_total, s2, s1, s3, n_subdomains, output_dim)
