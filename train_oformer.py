...
# from dataset_new import CFDBenchData
...
def build_model(flow_name, input_ch, output_ch=2):
    # input_chs = {'cavity':7, 'cylinder': 10, 'dam': 7, 'tube': 7}
    # input_ch = input_chs[flow_name]
...
if __name__ == '__main__':
    ...
    with tqdm(total=opt.iters) as pbar:
        ...
        while True:
            ...
            y = y.reshape(-1, 3, n_tolx, y.shape[-1])
            node_type = node_type.long().reshape(-1, 3, n_tolx, node_type.shape[-1])
            ...
            pred = decoder.forward(z, prop_pos, node_type[:,0,...].detach(), 3, input_pos)
